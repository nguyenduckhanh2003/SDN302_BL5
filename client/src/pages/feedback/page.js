...
const FeedbackPage = ({storeId}) => {
  ...
  const fetchFeedbacks = async (page = 1) => {
    try {
      ...
      const response = await axios.get(
        `/api/reviews/feedback?page=${page}`
      );
  ...
  const fetchReputation = async () => {
    try {
      setLoading(true);
      const response = await axios.get(
        `/api/reviews/reputation/${storeId}`
      );
      // console.log(response.data);
  ...
  return (
    <div className="p-4">
      <button onClick={handleOpenModal} className="text-xs text-[#0053A0] hover:underline block mt-3">
        See other items
      </button>
      {showModal && (
                  ...
                  {reputation ? (
                    <div className="flex justify-between items-center p-4 rounded-lg bg-white shadow-md">
                      <div className="flex flex-col items-center text-gray-600 font-medium">
                        ...
                        <span className="font-semibold text-gray-800 text-lg">{reputation.totalFeedbacks}</span>
                      </div>
                      <div className="flex flex-col items-center text-gray-600 font-medium">
                        <span className="text-sm">Positive Feedbacks</span>
                        <span className="font-semibold text-green-600 text-lg">{reputation.positiveFeedbacks}</span>
                      </div>
                      <div className="flex flex-col items-center text-gray-600 font-medium">
                        <span className="text-sm">Reputation Score</span>
                        <span className="font-bold text-blue-600 text-lg">{reputation.reputationScore}</span>
                      </div>
                    </div>
                  ) : (
                      ...
                      {feedbacks.map((feedback) => (
                        ...
                        >
                          <div className="flex items-center gap-2 text-yellow-500">
                            <FaStar />
                            <span className="font-medium">{feedback.rating} Stars</span>
                          </div>
